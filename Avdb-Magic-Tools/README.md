...
启用“JavDB 评分同步”后，打开影片详情页会读取当前 Emby 项目的 `ProviderIds.Num`，
通过 Avdb 后端按番号精确匹配 JavDB 影片，并将 JavDB 的 `movie.score` 写入 Emby
的 `CommunityRating`。该功能独立于剧照显示和补全，不会写入 `UserRating`；评分保存成功后详情页会自动刷新。

插件同时注册“JavDB 评分同步”计划任务，支持 Emby 的“立即运行”和默认每天 03:30 执行。
“允许 JavDB 评分覆盖已有社区评分”关闭时只填充空值或 0；无番号、无精确匹配、无评分、JavDB
账号失效、Avdb/JavDB 请求失败或保存失败时，都保留 Emby 原有评分，不写入空值或 0。
