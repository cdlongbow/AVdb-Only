...
关闭对应开关时只补缺，不替换已有图片。剧照任务不再由覆盖开关决定：只有 JavDB 有效剧照数量多于本地已有
附加剧照时才会删除旧图并全量重建，数量相等或远端更少时跳过。`OverwriteExistingBackdrop` 保留用于旧配置兼容，
不会绕过该数量规则。某个媒体库读取失败时会记录库 ID 并继续处理其他媒体库，完成日志会给出扫描、识别番号、
写入和失败统计。剧照按实际 Backdrop 索引读取，即使主背景缺失或索引中间有空洞，也不会隐藏后续已登记的剧照。

图片设置页中的 `启用影片剧照` 只控制详情页显示已经登记的剧照，不会触发补全或覆盖；
`启用剧照补全` 独立控制按需补全和“JavDB 剧照补全”计划任务。补全只有在远端有效剧照数量
多于本地已有附加剧照时才会替换，数量相等、更少、为空或查询失败时均保留本地剧照。
...
完整查询 JavDB `preview_images`，再将结果数量与影片当前登记的附加 Backdrop 数量进行对账；
...
当 JavDB 有效剧照数量多于本地已有附加剧照时，插件会在第一张合格 JavDB 剧照准备保存前删除旧的附加 `Backdrop`，
再从索引 1 开始写入本次完整检索得到的全部合格剧照；不会把新一批剧照继续追加到旧列表，因此重复打开预览不会不断
生成重复的 `fanartN`。数量一致或远端更少时不删除、不写入，索引为 0 的主背景图始终保留。检索结果为空、服务端不可用、
认证失败或返回不完整结果时，均保留原有内容，不会因为空结果清理旧的附加剧照；只有检索到至少一张有效 JavDB 剧照
且远端数量更多时才执行全量重建。客户端仍可逐步看到已经写入的剧照，不会因重新读取未写库的旧实体而反复覆盖同一个文件名。
...
后台轮询返回相同剧照索引时会复用当前滚动容器并保留横向位置，滑到后面的剧照不会因索引刷新跳回第一张。
...
预期输出为 `1`。最后在浏览器中强制刷新 Emby Web，打开一部 JavDB 有效剧照数量多于本地附加剧照的影片：
