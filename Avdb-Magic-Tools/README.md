...
标签栏采用 Emby“首页/收藏”同款的胶囊式横向滚动样式，背景阴影按标签内容宽度自适应；选中项使用更明显的高亮和阴影，宽屏会适当增加标签点击宽度。预告片隐藏选项已并入“主题 & UI”，可在同一标签中配置。
...
- 设置页标签按“图片 → 演员 → 主题 & UI → STRM → AVDB”排列，标签栏使用 Emby“首页/收藏”同款胶囊式横向滚动样式；背景和阴影跟随实际标签内容宽度，宽屏适当放大标签点击区域；“演员”标签合并演员写真/头像与演员墙两个设置区，栏目之间保留垂直间距和分隔线。
