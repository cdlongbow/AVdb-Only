...
数据库别名匹配和图片缓存。插件不会通过 AVDB 重复请求 TMDB，也不会自动覆盖现有演员头像。
...
地址。AVDB 返回的图片必须是同一已配置服务下的 Gfriends 图片代理地址；跨域地址、Jalbum
写真地址和其他路径都会被插件拒绝。管理员在 Emby 中选中头像后，下载和保存仍由 Emby 原生
远程图片流程完成。

AVDB 搜索接口只返回数据库中已经记录的 Gfriends 原图宽高，插件直接填写 Emby 原生
`RemoteImageInfo` 字段；没有宽高的候选会立即返回，不会在搜索请求内等待探测，也不会主动
请求未选中的图片。只有某张头像被图片代理实际请求并缓存后，AVDB 才从已缓存文件读取宽高并
写回数据库，下一次搜索即可显示该分辨率。尺寸字段不会进入客户端功能脚本，也不会把 API Key
写入图片地址。

如果不使用 AVDB 头像来源，可以关闭独立开关并安装
[龙王的头像来源插件](https://github.com/jzdxjk/Jav-Actors-Mapping/releases)。该外部插件与
Magic Tools 的 AVDB 头像提供者互不依赖。
...
现有的 Jalbum 数据库和图片缓存。插件设置中的 `AvdbApiBaseUrl` 必须由管理员明确填写，
例如 `http://10.0.0.3:8000`；留空、超时或没有匹配时均不显示写真，客户端和服务端都不会
回退到 GitHub CDN 或其他镜像。AVDB Token 和服务地址不会进入客户端脚本。
浏览器不会直接访问该地址，写真图片由已认证的
`GET /Plugins/AvdbMagicTools/Client/ActorStillsImage` 经 Emby 同源转发，因此 Emby
通过内网 IP、HTTPS 域名或嵌入式客户端访问时都可以继续使用内网 AVDB 地址。
与影片 `extrafanart` 的本地 Backdrop 接口、DOM 类名和画廊实例相互独立。

AVDB 索引只返回写真原图地址，写真网格和 Emby 原生全屏画廊共用同一份 AVDB 磁盘缓存，
...
Emby 同源代理直接流式返回 AVDB 下载的临时文件，不再把每张原图完整复制到托管内存，响应完成
或中断后自动删除临时文件。
整个链路不会访问 CDN 或写真文件树。
...
- 普通宽度下首页推荐图片继续使用约 0.55 秒的整帧渐隐渐出；超宽屏点击前后按钮时，三张
  主图按前后顺序在约 0.7 秒内滑入、滑出和换层。进入中间位置的新主图在封面运动完成前
  保持所有文字透明，到位后才依次显示 Logo 或标题、评分信息、简介和操作按钮。
